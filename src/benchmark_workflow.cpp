                                      ...
                                      const Mesh& m)
{
    ...
    bool rc = true;
    File f;
    if(f.open(obj_filename.string(), File::OM_RWC))
    {
        rc = rc && write_mesh_as_obj(f, m);
        f.close();
    }

    if(f.open(off_filename.string(), File::OM_RWC))
    {
                                            ...
                                            const RasterDouble& raster)
{
    ...
    File f;
    if(!f.open(raster_filename.string(), File::OM_RWC))
    {
        return false;
    }
    ...
    write_stats_row_callback write_stats_row)
{
    ...
    for(auto& method : available_methods)
    {
        ...
        for(int i = 0; i < parametrizations; i++)
        {
            ...
            File f;
            f.open(is_done_file.string(), File::OM_RWC);
        }
    }
    return true;
}
                                   ...
                                   const std::vector<int>& select_params)
{
    ...
    {
        if(!csv_file->open(benchmark_csv_filename.string(), File::OM_RW))
        {
    ...
    {
        if(!csv_file->open(benchmark_csv_filename.string(), File::OM_RWC))
        {
